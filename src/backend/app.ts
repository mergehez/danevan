...
export const app = {
    ...
    getTables: async (ps: { connectionId: number }): Promise<TableSummary[]> => {
        ...
        return withTimedAppOperation(`getTables:${ps.connectionId}`, { connectionId: ps.connectionId }, () => dbTools.getTables(ps.connectionId));
    },
    getTableInfo: async (ps: { connectionId: number; tableName: string }): Promise<TableInfo> => {
        ensureConnectionExists(ps.connectionId);
        appDb.touchConnectionLastUsed(ps.connectionId);
        return withTimedAppOperation(`getTableInfo:${ps.connectionId}:${ps.tableName}`, { connectionId: ps.connectionId, tableName: ps.tableName }, () =>
            dbTools.getTableInfo(ps.connectionId, ps.tableName)
        );
    },
    getTableDdl: async (ps: { connectionId: number; tableName: string }): Promise<string> => {
        ensureConnectionExists(ps.connectionId);
        appDb.touchConnectionLastUsed(ps.connectionId);
        return withTimedAppOperation(`getTableDdl:${ps.connectionId}:${ps.tableName}`, { connectionId: ps.connectionId, tableName: ps.tableName }, () =>
            dbTools.getTableDdl(ps.connectionId, ps.tableName)
        );
    },
    ...
    getTableData: async (ps: { connectionId: number; tableName: string; limit?: number; offset?: number; orderBy?: SortOrder; returnQuery?: boolean }): Promise<TableData> => {
        ...
        return withTimedAppOperation(
            `getTableData:${ps.connectionId}:${ps.tableName}`,
            {
