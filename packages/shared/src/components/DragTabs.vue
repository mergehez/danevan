...
import { computed, onMounted, onUnmounted, ref, watch, type ComponentPublicInstance, type UnwrapRef } from 'vue';
...
function useTab(item: Item) {
    ...
    function onDrop(e: DragEvent) {
        ...
        for (const curr of items2.value) {
            if (curr.data.hash == sourceItem.data.hash) continue;
...
function resolveWrapperElement() {
    ...
    return wrapperElement instanceof HTMLElement ? wrapperElement : undefined;
}

function init() {
    const wrapper = resolveWrapperElement();
    if (!wrapper) return;

    const tabEls = wrapper.querySelectorAll(`.${requiredClass}`);
    tabEls.forEach((tabEl) => {
        const id = tabEl.id;
        const item = items2.value.find((t) => t.data.hash === id);
...
<template>
    <TransitionGroup ref="wrapperRef" name="drag-tabs" tag="div">
        <template v-for="(e, i) in sortedItems" :key="e.data.hash">
