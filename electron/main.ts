...
const FRONTEND_DIST = join(__dirname, '..', 'apps', 'app', 'mainview', 'dist', 'index.html');
...
function createHandlers() {
    ...
    const backendHandlers: Record<string, (ps?: unknown) => Promise<unknown>> = {};
    for (const name of apiMethods) {
        const fn = (backendApp as Record<string, (ps?: unknown) => unknown>)[name];
        if (typeof fn === 'function') {
            backendHandlers[name] = async (ps?: unknown) => fn(ps);
        }
    }
